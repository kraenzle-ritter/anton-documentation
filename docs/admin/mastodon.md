...
Für die Einrichtung auf Anton benötigt man die Serveradresse (z.B. https://social.anoxinon.de) und die User-ID. Die User-Id kann man auf [prouser123.me/mastodon-userid-lookup/](https://prouser123.me/mastodon-userid-lookup/) nachschlagen. Anton kann die Timeline auf der Homepage links (`"home_left": true`) oder rechts (`"home_right": true`) unten einblenden oder eine eigene News Seite erstellen, die dann über in der Hauptnavigation erreichbar ist (`"extra_page": true`). Diese Angaben werden von uns dann im Setting `mastodon` gespeichert:
